...
if ($a == 'update')
{
    ...
    {
        cot::$db->delete($db_auth, "auth_groupid=$g");
    ...
    elseif ($auth = cot_import('auth', 'P', 'ARR'))
    {
        ...
        cot::$db->update($db_auth, array('auth_rights' => 0), "auth_groupid=$g");
        ...
        foreach ($auth as $k => $v)
        {
            foreach ($v as $i => $j)
            {
                if (is_array($j))
                {
                    ...
                    cot::$db->update($db_auth, array('auth_rights' => $mask),
...
$sql = cot::$db->query("SELECT a.*, u.user_name FROM $db_auth AS a 
